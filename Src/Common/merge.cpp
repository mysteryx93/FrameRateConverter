...
MergeFuncPtr getMergeFunc(int bits_per_pixel, int cpuSupport, BYTE* srcp, const BYTE* otherp, float weight_f, int& weight_i, int& invweight_i)
{
	...
	if (pixelsize == 1) {
		if ((cpuSupport >= ISET_AVX2) && IsPtrAligned(srcp, 32) && IsPtrAligned(otherp, 32))
			return &weighted_merge_planar_avx2;
		if ((cpuSupport >= ISET_SSE2) && IsPtrAligned(srcp, 16) && IsPtrAligned(otherp, 16))
			return &weighted_merge_planar_sse2;
#ifdef X86_32
		if (cpuSupport >= ISET_SSE)
...
static void merge_plane(BYTE* srcp, const BYTE* otherp, int src_pitch, int other_pitch, int src_rowsize, int src_height, float weight, int pixelsize, int bits_per_pixel, ICommonEnvironment& env) {
	int cpuSupport = env.GetCpuSupport();
	if ((weight > 0.4961f) && (weight < 0.5039f))
	{
		...
		if (pixelsize != 4) // 1 or 2
		{
			...
			else if ((cpuSupport >= ISET_SSE2) && IsPtrAligned(srcp, 16) && IsPtrAligned(otherp, 16)) {
				if (pixelsize == 1)
	...
	else
	{
		int weight_i;
		int invweight_i;
		MergeFuncPtr weighted_merge_planar = getMergeFunc(bits_per_pixel, cpuSupport, srcp, otherp, weight, /*out*/weight_i, /*out*/invweight_i);
...
//      int chroma_pitch = chroma->GetPitch(); 
//
//      if ((cpuSupport & CPUF_SSE2) && IsPtrAligned(chromap, 16) && IsPtrAligned(srcp, 16))
//      {
...
//    if (weight<0.9961f)
//    {
//      if ((cpuSupport & CPUF_SSE2) && IsPtrAligned(srcp, 16) && IsPtrAligned(lumap, 16))
//      {
